...
function EgovCdDetail(props) {
    console.group("EgovCdDetail");
    console.log("------------------------------");
    console.log("EgovCdDetail [props] : ", props);

    const navigate = useNavigate();
    const location = useLocation();
    console.log("EgovCdDetail [location] : ", location);
	//관리자 권한 체크때문에 추가(아래)
        ...
        EgovNet.requestFetch(deleteBoardURL,
            ...
            (resp) => {
                ...
                if (Number(resp.resultCode) === Number(CODE.RCV_SUCCESS)) {
                    alert("게시글이 삭제되었습니다.")
                    navigate(URL.INFORM_CD ,{ replace: true });
                } else {
                            ...
                            <div className="board_btn_area">
                                {user && sessionUserSe ==='ADM' && masterBoard.bbsUseFlag === 'Y' &&
                                    <div className="left_col btn3">
                                        <Link to={{pathname: URL.INFORM_CD_MODIFY}}
                                            state={{
                                                nttId: nttId,
                                                bbsId: bbsId
                                            }}
                                        ...
                                        <Link to={{pathname: URL.INFORM_CD_REPLY}}
                                            state={{
                                                nttId: nttId,
                                                bbsId: bbsId
                                            }}
                                ...
                                <div className="right_col btn1">
                                    <Link to={{pathname: URL.INFORM_CD}}
                                        state={{
                                            nttId: nttId,
                                            bbsId: bbsId,
