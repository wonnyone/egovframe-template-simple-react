...
function EgovCntcEdit(props) {
    console.group("EgovCntcEdit");
    console.log("------------------------------");
    console.log("EgovCntcEdit [props] : ", props);

    const navigate = useNavigate();
    const location = useLocation();
    console.log("EgovCntcEdit [location] : ", location);
	//관리자 권한 체크때문에 추가(아래)
            ...
            EgovNet.requestFetch(modeInfo.editURL,
                ...
                (resp) => {
                    if (Number(resp.resultCode) === Number(CODE.RCV_SUCCESS)) {
                        navigate(URL.INFORM_CNTC, {state:{bbsId : bbsId}});
                    } else {
                        ...
                        <div className="board_view2">
                            ...
                            {sessionUserSe ==='ADM' &&
                                ...
                                <div className="right_col btn1">
                                    <Link to={URL.INFORM_CNTC} className="btn btn_blue_h46 w_100">목록</Link>
                                </div>
                            </div>
                            {/* <!--// 버튼영역 --> */}
                        </div>
