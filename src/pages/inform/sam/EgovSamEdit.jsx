...
function EgovSamEdit(props) {
    console.group("EgovSamEdit");
    console.log("------------------------------");
    console.log("EgovSamEdit [props] : ", props);

    const navigate = useNavigate();
    const location = useLocation();
    console.log("EgovSamEdit [location] : ", location);
	//관리자 권한 체크때문에 추가(아래)
            ...
            EgovNet.requestFetch(modeInfo.editURL,
                ...
                (resp) => {
                    if (Number(resp.resultCode) === Number(CODE.RCV_SUCCESS)) {
                        navigate(URL.INFORM_SAM, {state:{bbsId : bbsId}});
                    } else {
                        ...
                        <div className="board_view2">
                            ...
                            {sessionUserSe ==='ADM' &&
                                ...
                                <div className="right_col btn1">
                                    <Link to={URL.INFORM_SAM} className="btn btn_blue_h46 w_100">목록</Link>
                                </div>
                            </div>
                            {/* <!--// 버튼영역 --> */}
                        </div>
